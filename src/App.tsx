...
function PrinterSettingsModal({ settings, taxSettings, displaySettings, githubSettings, onSave, onSaveTax, onSaveDisplay, onSaveGithub, onClose }){
  ...
  // ── Receipt style (paper width, fonts, etc.) ────────────────
  const DEFAULT_RECEIPT_STYLE = {
    paperWidth: 80, fontSize: 13, fontFamily: "monospace",
    headerAlign: "center", showDivider: true, dividerChar: "─",
    boldTotal: true, showOrderNo: true, showDateTime: true, showTaxId: true, footerAlign: "center",
  };
...
// ─────────────────────────────────────────────────────────────
// Simple list with move-up/move-down buttons (replaces full drag-and-drop)
function DraggableGroupList({ groups, setGroups, onUpdate, onDelete }){
  function move(idx, dir){
    setGroups(prev=>{
      const arr=[...prev];
      const j=idx+dir;
...
function App(){
  ...
  // Sync only menu catalog + sales stats across devices
  // Settings (printer, tax, display) are per-device — each terminal has its own printer
  useSharedSync({
    menuItems:setMenuItems, categories:setCategories, catInfo:setCatInfo,
    modGroups:setModGroups, orders:setOrders,
  });
